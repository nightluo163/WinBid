...
def search(keyword, start_time):
    ...
    home_url = "https://www.ebidding.com"
    try:
        home_response = session.get(home_url)
        home_response.raise_for_status()

    except Exception as e:
            logger.error(f"国e平台，主页请求失败: {str(e)}")
            return None
    ...
    api_url = f"https://www.ebidding.com/.rest/delivery/announcement/?searchContent%5Blike%5D={quote(keyword, encoding='utf-8')}"
    bid_list = []
    try:
        ...
        for list in data_list:
            ...
            if bid_time >= start_time.replace(tzinfo=None):
                bid = {
                    "标题": list['title'],
                    "链接":  f"https://www.ebidding.com/e-portal/business/detail.html?id={list['pkId']}"
                }
                bid_list.append(bid)
            else:
                break
        ...
        logger.error(f"国e平台，API请求失败: 状态码 {response.status_code}, 响应内容: {response.text}")
        return None
...
def lambda_handler(event, context):
    ...
    while beijing_time <= end_time:
        try:
            ...
            logger.error(f"国e平台，全局异常: {str(e)}")
            error_send = webhook_test.send_text(f"国e平台，全局异常: {str(e)}")
